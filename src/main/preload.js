...
contextBridge.exposeInMainWorld('pkmn', {
  ...
  onGymLeaderState: (cb) => ipcRenderer.on('gym-leader-state', (_e, payload) => cb(payload)),
  acceptGymLeader: (id) => ipcRenderer.send('pkmn:accept-gym-leader', String(id)),
  dismissGymLeader: (id) => ipcRenderer.send('pkmn:dismiss-gym-leader', String(id)),
