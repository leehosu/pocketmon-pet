...
- 상태는 `~/.pocketmon/save.json`에 **평범한 JSON**으로 저장됩니다. level/stage는 저장값을 신뢰하지 않고 항상 xp에서 재계산합니다(손상·실수 방지). 파일이 손상되면 `save.json.bak`으로 백업 후 초기화합니다.
- 개인용 로컬 앱이라 **치팅 방지(서명)는 두지 않습니다** — 어차피 키·계산이 모두 사용자 기기에 있어 로컬만으로는 소유자의 조작을 막을 수 없습니다. "특정 주체만 상태를 바꿀 수 있게"(예: 리더보드) 하려면 서버 권위 방식이 필요합니다.
- **완전 초기화**: 앱을 종료하고 `~/.pocketmon/save.json`을 삭제한 뒤 재실행하면 새 포켓몬이 알부터 다시 시작됩니다. `~/.pocketmon/offset`도 함께 삭제 권장.
