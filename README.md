...
- **146종의 야생 포켓몬이 등장합니다.** 원작 Gold 출현 데이터와 플레이어 레벨을 함께 반영합니다.
- **체육관 진행도가 저장됩니다.** 현재는 비상 2연전과 윙배지 획득까지 구현되어 있습니다.
...
- 선택된 포켓몬과 레벨·전적·배지는 `~/.pocketmon/save.json`에 저장되므로 재실행해도 유지됩니다.
...
- 승리해 윙배지를 얻으면 같은 관장은 다시 출현하지 않습니다.
...
- 야생전과 관장전 모두 Gen 2 능력치·타입 상성·명중률·급소·상태이상 계산을 사용합니다.
