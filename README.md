...
훅은 이벤트를 `~/.pocketmon/events.jsonl`에 append만 하고,
앱이 이를 4초 간격(tick)으로 읽어 XP와 애니메이션에 반영합니다.
...
- 상태는 `~/.pocketmon/save.json`에 **평범한 JSON**으로 저장됩니다. level/stage는
  저장값을 신뢰하지 않고 항상 xp에서 재계산합니다(손상·실수 방지). 파일이 손상되면
  `save.json.bak`으로 백업 후 초기화합니다.
- 개인용 로컬 앱이라 **치팅 방지(서명)는 두지 않습니다** — 어차피 키·계산이 모두
  사용자 기기에 있어 로컬만으로는 소유자의 조작을 막을 수 없습니다. "특정 주체만
  상태를 바꿀 수 있게"(예: 리더보드) 하려면 서버 권위 방식이 필요합니다.
- **완전 초기화**: 앱을 종료하고 `~/.pocketmon/save.json`을 삭제한 뒤 재실행하면
  새 포켓몬이 알부터 다시 시작됩니다. `~/.pocketmon/offset`도 함께 삭제 권장.
...
- 포켓몬 스프라이트는 앱에 **번들하지 않고**, 공개 [PokéAPI/sprites](https://github.com/PokeAPI/sprites)에서
  **런타임에 내려받아** `~/.pocketmon/sprites/`에 로컬 캐시합니다. (트레이 메뉴 "포켓몬 스프라이트 받기"
...
- 울음소리도 동일하게 공개 [PokéAPI/cries](https://github.com/PokeAPI/cries)에서 런타임에 받아
  `~/.pocketmon/cries/`에 캐시하고, 펫을 클릭하면 재생합니다(앱 번들 미포함).
...
전혀 없는** 개인·비상업용 팬 프로젝트입니다. 소스 코드는 자유롭게 사용할 수 있으나, "포켓몬" 및 관련
명칭·캐릭터·스프라이트 등 제3자 상표/저작물의 권리는 각 권리자에게 있으며 본 프로젝트는 그에 대한
어떤 권리도 주장하지 않습니다. 스프라이트 등 IP 에셋은 앱에 포함하지 않고 런타임에 외부 공개 소스에서
받아 개인 기기에 캐시할 뿐입니다. 권리자의 요청이 있으면 즉시 관련 기능을 제거합니다.
