...
- 중앙 포켓몬은 원본 사용자 기준점 `(48, 84)`에 배치
- 상대 기준점 `(136, 56)`의 수평 거리는 화면 짧은 변의 42%가 되도록 공통 좌표 배율을 계산
- 같은 좌표 배율을 X/Y 이동에 적용해 원본 오른쪽 위 방향과 오브젝트 궤적을 유지
- 오브젝트 타일은 화면 짧은 변에 따라 2~5배 정수 확대하고, 포켓몬은 별도로 2~3배만 확대
- 포켓몬은 계속 화면 정중앙에 두며 상대 포켓몬이나 타깃 표시는 그리지 않음
- 기술별 위치·크기 예외 없이 모든 기술에 같은 반응형 변환을 적용
...
- 24개 기술 접촉 시트에서 빈 화면, 잘린 타일, 비정수 확대, 포켓몬과 타일의 과도한 겹침 확인
- 640x480과 실제 디스플레이 크기에서 상대 기준점이 짧은 변의 42% 거리에 놓이는지 검사
